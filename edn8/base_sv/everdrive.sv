...
//**************************************************************************************** data bus drivers
  ...
  reg [15:0]sha_written_addr;
  reg [7:0]sha_written_data;

  always @(negedge m2) begin
    case(sha_watch_state)
      0: begin
        if(cpu_rw && cpu_dato[7:0] == 8'hA2 /* LDX imm */) begin
          sha_watch_state <= 1;
        end
      end
      1: begin
        if(cpu_rw && cpu_dato[7:0] == 8'hAA /* LDX #$AA */) begin
          sha_watch_state <= 2;
        end else begin
          sha_watch_state <= 0;
        end
      ...
      2: begin
        if(cpu_rw && cpu_dato[7:0] == 8'hA0 /* LDY imm */) begin
          sha_watch_state <= 3;
        end else begin
          sha_watch_state <= 0;
        end
      ...
      3: begin
        if(cpu_rw && cpu_dato[7:0] == 8'h10 /* LDY #$10 */) begin
          sha_watch_state <= 4;
        end else begin
          sha_watch_state <= 0;
        end
      ...
      4: begin
        ...
        sha_watch_state <= 5; // 5 for SHA ($30), Y, 6 for SHA $1EF0, Y
      end
      5: begin
      ...
      9: begin
        ...
        // Time to snoop!
        sha_written_addr[15:0] <= {!cpu_ce, cpu_addr[14:0]};
        sha_written_data[7:0] <= cpu_dato[7:0];
        sha_watch_state <= 0;
      end
    endcase
  end
